...
import { getProjectMemoryFile, getProjectCowrnglrMd } from "./project_context.js";
...
export class Agent {
  ...
  private _buildSystemPrompt(basePrompt: string): string {
    ...
    const cowrnglrMdPath = getProjectCowrnglrMd()
    if (fs.existsSync(cowrnglrMdPath)) {
      const cowrnglrContent = fs.readFileSync(cowrnglrMdPath, "utf-8").trim();
      if (cowrnglrContent) {
        finalPrompt +=
    ...
    const memoryPath = getProjectMemoryFile()
    ...
    // Keşif indeksi: tüm global/bundled/local skill'ler (yalnız id + açıklama).
  ...
  private getTools() {
    ...
    // olayları yayınla. Böylece her araç kendi loader→checkmark geçişini ayrı ayrı yapar
    // (toplu/batch değil). id = SDK'nın toolCallId'si (benzersiz).
  ...
  async chat(
    ...
    // Araç callback'lerini aktif chat için kaydet — getTools() sarmalayıcısı bunları okur.
            ...
            onStepFinish: async ({ text, toolCalls, usage, providerMetadata }: any) => {
              ...
              if (toolCalls && toolCalls.length > 0) {
                for (const call of toolCalls) {
                  ...
                  // NOT: UI start/done olayları artık getTools() execute sarmalayıcısından
                  // gerçek zamanlı ve araç-bazlı yayınlanıyor (onToolCall / onToolEvent).
                }
              }
