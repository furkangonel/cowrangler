...
export const useSessionsStore = create<SessionsState>((set, get) => ({
  ...
  loadMessages: async (sessionId) => {
    ...
    try {
      ...
      // Önce bir önceki oturumdan kalan CANLI tool call'ları temizle — yoksa
      // önceki oturumda çağrılan utilize_skill sağ panel Context'ine sızıyor.
  ...
  setActiveSession: (id) => {
    ...
    // Oturum her değiştiğinde agent.store'daki CANLI tool call + timeline'ları
    // temizle ki bir önceki oturumun skill/tool bağlamı yeni oturuma sızmasın.
    // (Sidebar dışı geçiş yolları — ProjectHome listesi, CodeSessionView — de
