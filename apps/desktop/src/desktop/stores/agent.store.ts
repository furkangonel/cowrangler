    ...
    const unsubQa = ipc.agent.onQaPrompt(async (payload: any) => {
      ...
      if (payload?.meta?.sessionId) {
        try {
          ...
          const currentCodeSessionId = useUIStore.getState().activeCodeSessionId

          if (payload.meta.sessionId !== currentProjectSessionId && payload.meta.sessionId !== currentCodeSessionId) {
