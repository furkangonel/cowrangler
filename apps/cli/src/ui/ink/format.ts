import os from "os";
const HOME = os.homedir();

const TOOL_LABEL_WIDTH = 18;

/**
 * Map of tool name → which arg keys are worth surfacing in the progress
 * trace line. Mirrors the legacy spinner.ts table so we don't lose
 * cherry-picked argument display.
 */
const TOOL_ARG_KEYS: Record<string, string[]> = {
  read_file: ["path"],
  write_file: ["path"],
  edit_file: ["path"],
  append_to_file: ["path"],
  copy_file: ["source"],
  delete_file: ["path"],
  delete_folder: ["path"],
  list_files: ["path"],
  glob_files: ["pattern"],
  grep_files: ["pattern", "path"],
  file_info: ["path"],
  execute_bash: ["command"],
  fetch_webpage: ["url"],
  http_request: ["method", "url"],
  web_search: ["query"],
  git_status: [],
  git_diff: ["path"],
  git_log: [],
  git_add: ["paths"],
  git_commit: ["message"],
  git_branch: ["action", "name"],
  git_stash: ["action"],
  git_checkout_file: ["path"],
  spawn_subagent: ["agentType", "taskDescription"],
  spawn_subagent_parallel: ["tasks"],
  write_plan: ["title"],
  notify: ["message"],
  notebook_edit: ["notebook_path", "action"],
  search_in_files: ["keyword", "path"],
  create_folder: ["path"],
  move_item: ["source", "destination"],
  create_pdf: ["path"],
  utilize_skill: ["skill_name"],
  create_skill: ["skill_id"],
  list_skills: [],
  get_system_info: [],
  which_command: ["command"],
  manage_todo: ["action"],
  sleep: ["ms"],
  send_message: ["message"],
};

export function formatArgs(
  toolName: string,
  args?: Record<string, any>,
): string {
  if (!args) return "";
  const keys = TOOL_ARG_KEYS[toolName] ?? Object.keys(args).slice(0, 2);
  const parts: string[] = [];
  for (const k of keys) {
    const v = args[k];
    if (v === undefined || v === null) continue;
    let str = Array.isArray(v)
      ? v.join(", ")
      : typeof v === "string"
        ? v
        : JSON.stringify(v);
    
    // Shorten HOME prefix if it is a path key
    if (typeof v === "string" && (k === "path" || k === "source" || k === "destination")) {
      if (str.startsWith(HOME)) {
        str = "~" + str.slice(HOME.length);
      }
    }

    if (str.length > 48) str = str.slice(0, 45) + "…";
    parts.push(str);
  }

  // read_file offset/limit special formatting
  if (toolName === "read_file" && args.offset !== undefined && args.limit !== undefined) {
    const start = args.offset;
    const end = args.offset + args.limit - 1;
    let pathVal = args.path || "";
    if (pathVal.startsWith(HOME)) {
      pathVal = "~" + pathVal.slice(HOME.length);
    }
    return `${pathVal}:${start}-${end}`;
  }

  return parts.join("  ");
}

export function padToolName(name: string): string {
  return name.length >= TOOL_LABEL_WIDTH
    ? name
    : name + " ".repeat(TOOL_LABEL_WIDTH - name.length);
}

export function truncateNarrative(text: string, max = 120): string {
  return text.length > max ? text.slice(0, max - 3) + "…" : text;
}

export function formatElapsed(ms: number): string {
  return (ms / 1000).toFixed(1) + "s";
}
