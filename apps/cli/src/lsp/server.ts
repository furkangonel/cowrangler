...
import { getLogger } from "@cowrangler/core/logger.js";
...
const COWRANGLER_KEYWORDS = [
  ...
  { label: "/login", kind: 14, detail: "Sign in with a subscription (Claude Pro, ChatGPT, Copilot, Gemini, Antigravity)" },
  { label: "/terse", kind: 14, detail: "Toggle token-efficient output mode" },
  { label: "/plan", kind: 14, detail: "Enter read-only plan mode" },
  { label: "/act", kind: 14, detail: "Exit plan mode (edits enabled)" },
  { label: "/undo", kind: 14, detail: "Revert the last file change" },
  { label: "/checkpoints", kind: 14, detail: "File change history" },
  { label: "/goal", kind: 14, detail: "Autonomous goal mode" },
  { label: "/recipe", kind: 14, detail: "Run a parametrized workflow recipe" },
  { label: "/copy", kind: 14, detail: "Copy last response to clipboard" },
