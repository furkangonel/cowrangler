...
export async function runModelPicker(): Promise<void> {
  ...
  // Models contributed by installed plugins (e.g. Antigravity), so they show
  // in the picker after install without needing to be saved manually.
  let pluginModels: string[] = [];
  try {
    const { PluginManager } = await import("@cowrangler/core/plugins.js");
    pluginModels = PluginManager.getInstance().getPluginModels();
  } catch { /* plugins optional */ }

  const baseModelIds = savedModels.length > 0 ? savedModels : DEFAULT_MODELS.map((d) => d.model);
  const mergedIds = [...new Set([...baseModelIds, ...pluginModels])];
  const baseList = mergedIds.map((m) => {
    const found = DEFAULT_MODELS.find((d) => d.model === m);
    if (found) return found;
    const provider = m.includes("/") ? m.split("/")[0] : "Custom";
    return { model: m, provider, envKey: "" };
  });
