...
export function getModelMeta(model: string): ModelMeta | null {
  ...
  for (const [key, meta] of Object.entries(MODEL_REGISTRY)) {
    if (model.includes(key) || (normalized.length > 6 && key.includes(normalized))) return meta;
  }

  // Fallback to ensure getModelMeta never returns null for slash-separated unknown models
  if (model.includes("/")) {
    const rawProvider = model.split("/")[0] || "openai";
    const knownProviders = ["openai", "anthropic", "google", "groq", "vertex", "copilot", "mistral", "deepseek", "xai", "together", "cerebras", "fireworks", "ollama", "lmstudio", "local"];
    const provider = knownProviders.includes(rawProvider) ? rawProvider : "openrouter";
...
export function classifyProviderAuth(model: string, env: Record<string, string>): { kind: "ok" | "missing"; env?: string } {
  const provider = model.split("/")[0] || "openai";
  if (provider === "openai" && !env.OPENAI_API_KEY) {
    return { kind: "missing", env: "OPENAI_API_KEY" };
  }
  if (provider === "anthropic" && !env.ANTHROPIC_API_KEY) {
    return { kind: "missing", env: "ANTHROPIC_API_KEY" };
  }
  return { kind: "ok" };
}

export function registerDynamicModel(modelId: string, metadata: ModelMeta) {
  MODEL_REGISTRY[modelId] = metadata;
  if (modelId.includes("/")) {
